...
!	Far-UV:  Eq. 5
  ...
  FUNCTION EFLUX_SCALAR_VECTOR_(filter_ids, wlength, flux, z) RESULT(EFLUX)
    ...
    EFLUX = MEAN(flux_, weights=wl_res*res)

	END FUNCTION

  FUNCTION EFLUX_SCALAR_MATRIX_(filter_ids, wlength, flux, z) RESULT(EFLUX)
    ...
    where(.not.masked)
      EFLUX = MEAN(flux_, weights=spread(wl_res*res, 2, size(flux, 2)), dim=1)
    end where

	END FUNCTION

	FUNCTION EFLUX_VECTOR_VECTOR_(filter_ids, wlength, flux, z) RESULT(EFLUX)
    ...
    do j=1, size(filter_ids)
      ...
      EFLUX(j) = MEAN(flux_, weights=wl_res(k:l)*res(k:l))

    end do

	END FUNCTION

	FUNCTION EFLUX_VECTOR_MATRIX_(filter_ids, wlength, flux, z) RESULT(EFLUX)
    ...
    EFLUX = 0.0
		...
		CALL GET_FILTER(filter_ids, wl_res, res, i, echo=.false.)
    res_ = spread(wl_res * res, 2, size(flux, 2))
  ...
  SUBROUTINE READ_BC_FILTERS_(echo)
    ...
    logical, intent(in)  :: echo
		...
		CALL GETENV("FILTERS", filters_bin)
      ...
      vals_ = achar(0)
		...
		do i = 1, narg
			...
			case(2)
				...
				if(.not. parse_long) then
					print*, "STOP PARSE_ARGS: invalid option '", args_(i)(iopt:fopt), "'."
					stop
				end if

				mask     = index(long_opts, args_(i)(iopt:fopt)) == 1
...
!                corresponding domain boundary is given instead.
  ...
  SUBROUTINE VELOCITY_DISP(wlength, fssp, fluxo, sigma, wl_range, vel_disp)
...
!   * vel_disp : real value of the computed LOSVD.
    ...
    best_chi = BRENT(40.0, (450.0 + 40.0) * 0.5, 450.0, CHI_SQUARE, 1e-4, vel_disp)
    ...
    FUNCTION CHI_SQUARE(losvd) RESULT(CHISQ)
      ...
      where(spread(mask, 2, nages)) fssp_vdsig = fssp_vd / spread(sigma(k(1):k(2)), 2, nages)
      where(mask) fluxo_s = fluxo(k(1):k(2)) / sigma(k(1):k(2))
